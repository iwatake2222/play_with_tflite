...
int32_t FeatureEngine::Process(const cv::Mat& original_mat, const std::vector<BoundingBox>& bbox_list, Result& result)
{
    if (!inference_helper_) {
        PRINT_E("Inference helper is not created\n");
        return kRetErr;
    ...
    result.feature_list.reserve(bbox_list.size());
    for (const auto& bbox : bbox_list) {
        ...
        std::copy(raw_feature_list, raw_feature_list + feature.size(), feature.data());
        const auto& t_post_process1 = std::chrono::steady_clock::now();

        result.feature_list.push_back(feature);
