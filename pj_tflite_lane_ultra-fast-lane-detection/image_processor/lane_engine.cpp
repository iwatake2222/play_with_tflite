...
static constexpr int32_t kNumLine = 4;
#endif
//static constexpr int32_t kNumGriding = 5;
//static constexpr int32_t kNumClassPerLine = 3;
//static constexpr int32_t kNumLine = 2;

static constexpr int32_t kNumHeight = 800;
static constexpr int32_t kNumWidth = 288;
...
static inline float& GetValue(std::vector<float>& val_list, int32_t num_i, int32_t num_j, int32_t num_k, int32_t i, int32_t j, int32_t k)
{
    return val_list.at(i * (num_j * num_k) + j * num_k + k);

}

static inline void PrintValue(std::vector<float>& val_list, int32_t num_i, int32_t num_j, int32_t num_k)
{
    for (int32_t i = 0; i < num_i; i++) {
        for (int32_t j = 0; j < num_j; j++) {
            for (int32_t k = 0; k < num_k; k++) {
                printf("%.12f, ", val_list.at(i * (num_j * num_k) + j * num_k + k));
            }
            printf("\n");
        }
        printf("---\n");
...
static inline std::vector<float> Softmax_0(const std::vector<float>& val_list, int32_t num_i, int32_t num_j, int32_t num_k)
{
...
static inline std::vector<float> MulSum(const std::vector<float>& val_list_0, const std::vector<float>& val_list_1, int32_t num_i, int32_t num_j, int32_t num_k)
{
...
int32_t LaneEngine::Process(const cv::Mat& original_mat, Result& result)
{
    ...
    /* Retrieve the result */
    std::vector<float> output_raw_val(output_tensor_info_list_[0].GetDataAsFloat(), output_tensor_info_list_[0].GetDataAsFloat() + output_tensor_info_list_[0].GetElementNum());
    //for (int32_t i = 0; i < output_raw_val.size(); i++) {
    //    printf("%f,", output_raw_val[i]);
    //}


    std::vector<double> linSpaceVector = linspace(0, kNumHeight - 1, kNumGriding - 1);
    double linSpace = linSpaceVector[1] - linSpaceVector[0];

    //PrintValue(output_raw_val, kNumGriding, kNumClassPerLine, kNumLine);
    ...
    printf("\nprob\n");
    //PrintValue(prob, kNumGriding - 1, kNumClassPerLine, kNumLine);
    ...
    printf("\loc\n");
    //PrintValue(loc, 1, kNumClassPerLine, kNumLine);
    ...
    for (int32_t k = 0; k < kNumLine; k++) {
        for (int32_t j = 0; j < kNumClassPerLine; j++) {
            float val = loc.at(j * kNumLine + k);
            bool is_valid = valid_map.at(j * kNumLine + k);
            if (is_valid && val > 0) {
                int32_t x = val * linSpace;
                int32_t y = culane_row_anchor[kNumClassPerLine - 1 - j];
                cv::circle(img_src, cv::Point(x, y), 2, cv::Scalar(0, 255, 0), 2);
            }
        }
    }
